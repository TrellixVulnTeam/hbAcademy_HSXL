-- Consideration: for cat table or dog table, animal_id is FK for them
-- Cat (cat_id, name, weight, age, animal_id (fk), shelter_id (fk), adopted_id (fk) )
    -- this can be used to confirm correct type within type tables (dog, cat tables)
    -- ex: animal_id would read like: 1 = cat, 2 = dog, 3 = bird
    -- as well as belonging to a shelter
    -- and belonging to a user via adopted_id association

--  === Cat table ===
-- id (pk)
-- name (first and last?) : varchar(30)
-- age : integer
-- weight : float
-- shelter_id : integer

-- the species will merge cat and dog and add the id as differentiator.
-- example primary id: 1 is an actual cat's unique id
-- species_id: 1 is the identifier that it is a cat
